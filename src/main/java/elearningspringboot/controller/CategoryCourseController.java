...
public class CategoryCourseController {
    ...
    public ResponseEntity<ResponseData<PageResponse<List<CategoryCourseResponse>>>> getAll(
        ...
        log.info("Request: Get categories with pageNumber={}, pageSize={}, sorts={}, keyword={}", pageNumber, pageSize, sorts, keyword);
        PageResponse<List<CategoryCourseResponse>> response = service.getCategoriesWithPaginationAndKeyword(pageNumber, pageSize, sorts, keyword);
        String message = messageSource.getMessage("categoryCourse.get.list.success", null, LocaleContextHolder.getLocale());
        return ResponseBuilder.withData(HttpStatus.OK, message, response);
    }

    @GetMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ResponseData<CategoryCourseResponse>> getById(@PathVariable("id") @Min(value = 1, message = "{validation.id.min}") Long id) {
        log.info("Request: Get category course by id = {}", id);
        CategoryCourseResponse dto = service.getById(id);
        String message = messageSource.getMessage("categoryCourse.get.success", null, LocaleContextHolder.getLocale());
        return ResponseBuilder.withData(HttpStatus.OK, message, dto);
    }

    @PostMapping
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ResponseData<CategoryCourseResponse>> create(@Validated({OnCreate.class, Default.class}) @RequestBody CategoryCourseRequest request) {
        log.info("Request: Create category course = {}", request);
        CategoryCourseResponse dto = service.create(request);
        String message = messageSource.getMessage("categoryCourse.create.success", null, LocaleContextHolder.getLocale());
        return ResponseBuilder.withData(HttpStatus.CREATED, message, dto);
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ResponseData<CategoryCourseResponse>> update(@PathVariable("id") @Min(value = 1, message = "{validation.id.min}") Long id,
        ...
        String message = messageSource.getMessage("categoryCourse.update.success", null, LocaleContextHolder.getLocale());
        return ResponseBuilder.withData(HttpStatus.OK, message, dto);
    }
    ...
    public ResponseEntity<ResponseData<Void>> delete(@PathVariable("id") @Min(value = 1, message = "{validation.id.min}") Long id) {
        ...
        service.delete(id);
        String message = messageSource.getMessage("categoryCourse.delete.success", null, LocaleContextHolder.getLocale());
